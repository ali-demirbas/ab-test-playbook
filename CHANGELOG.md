...
### Added
- 6 more scenarios (199 → 205): loss-vs-gain message framing at trial end, an external reference price shown before the actual price, pre-purchase product customization, country-of-origin display, bundle price-range composition, and a deliberately designed "peak" moment on the thank-you page. Three of these (loss framing, reference-price anchoring, customization/ownership) were identified as gaps in an earlier pass over this project and never actually added until now — caught on a second check, not assumed clean.
- 2 final scenarios (197 → 199): CTA copy mood (imperative vs. first-person) and a privacy-reassurance line next to a lead-capture field. Closes this pass — every remaining candidate checked against the archive either already existed under a different name, or was cosmetic-tier (color/shape/font) and deliberately skipped per methodology.md's own impact ranking.
- 7 more scenarios (190 → 197), each a mechanism check that came up empty against the existing archive: social proof counters, a partial-screen welcome overlay, pricing-plan display order, visible payment-method icons before checkout, a small priming question ahead of the main form, superscript price formatting, and opening external links in a new tab. Distributed across `home-landing.md`, `pricing.md`, `cart-checkout.md`, `forms-signup.md` and `ui-elements.md`.
- `knowledge/scenarios/thank-you.md` and `knowledge/scenarios/dashboard.md` — two journey stages the archive had no coverage for at all (post-purchase/post-signup confirmation, and the logged-in recurring-use home screen, as distinct from first-open onboarding or the marketing homepage). 3 scenarios each, plus one out-of-stock-display scenario added to each of `product-detail.md` and `category-listing.md` (182 → 190). The router's page-to-file mapping (`ab-test-suggest/SKILL.md`) and all scenario-count references were updated to match.
- 3 new scenarios (179 → 182), each covering a mechanism the archive had no scenario for at all — checked against the existing archive first, not assumed: **decoy-effect pricing** (`pricing.md` — a third, deliberately-less-attractive plan shifts which of the other two gets picked; distinct from the existing plan-count scenario, which is about how many options to show, not about one option's role being comparison-anchor rather than a real seller), **labor-illusion checkout loading** (`cart-checkout.md` — a step-by-step "verifying card / awaiting bank approval" loading screen vs. a plain spinner; guarded explicitly against artificially slowing down the real process to fake more "work," which would cross into rule 6), and **Zeigarnik-effect profile completion** (`mobile-app.md` — a persistent "70% complete" nudge on return visits; distinct from the existing checkout progress-bar scenario, which orients the user mid-flow rather than pulling them back afterward). All three pass `validate_scenarios.py` on the existing format rules unchanged. Scenario-count references updated across both READMEs, `docs/architecture.md` and `docs/index.html`.
