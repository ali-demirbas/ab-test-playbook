...
**A second pitfall, separate from the first: don't read "significant in one segment, not in another" as itself proof the effect differs by segment.** `analyze_results.py significance` run once on mobile and once on desktop answers two separate questions — it doesn't test whether mobile's effect and desktop's effect are statistically distinguishable from each other. Mobile coming back significant and desktop not is exactly what you'd expect to see by chance even when the true effect is identical in both segments, especially with an uneven sample split between them; the tool has no built-in interaction test to tell the difference. So a segment breakdown from this playbook is read as **exploratory, not confirmatory**: it can surface a hypothesis worth a dedicated follow-up test (methodology.md → prioritization feeds the next `ab-test-suggest`/`ab-test-design` round), but it doesn't get reported as "B won on mobile" the way an overall, pre-specified result does. If the user needs an actual, decision-grade claim about whether a segment interaction is real, that's a formal interaction test — outside what this playbook computes — and the output says so rather than implying the two per-segment z-tests already answered it.
...
**This escape tactic is a different kind of experiment, and it's worth naming that difference rather than blurring it with rule 4.** A **diagnostic experiment** isolates one variable so a win or loss teaches you which specific mechanism moved behavior — this is what rule 4 (one variable) governs, and it's the default. A **radical-redesign experiment** bundles many changes into one new experience on purpose; it can tell you whether the new experience as a whole beats the old one, but it can't tell you which of the bundled changes did the work — that's the price of measuring the total effect. When this escape tactic is used, say explicitly that it's a bundled-treatment test, not a rule-4 single-variable test wearing a bigger variant: the result answers "is the new experience better," not "which part of it worked." A follow-up round of single-variable tests is what answers that second question, once the bundle has won.
